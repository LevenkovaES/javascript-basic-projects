...
const form: HTMLElement | null = document.querySelector(".lorem-form");
form?.addEventListener("submit", displayText);

function displayText(e: Event): void {
  e.preventDefault();

  const loremText: HTMLElement | null = document.querySelector(".lorem-text");
  ...
  if (amount < 1) {
    paragraphs = createParagraph(text[createNRandomNumber(text.length)]);
  } else if (amount > 9) {
    paragraphs = createParagraph("The number of paragraphs is too large");
  } else {
    paragraphs = document.createElement("div");

    const tempArr = text.slice(0, amount);

    tempArr.forEach((string: string): void => {
      paragraphs.append(createParagraph(string));
    });
  }

  loremText.replaceChildren(paragraphs);

  function createNRandomNumber(range: number): number {
    return Math.floor(Math.random() * range);
  }

  function createParagraph(textContent: string): HTMLElement {
